...
def get_mat(x, y, z):
    ...
    K = jnp.array([[0, -v3[2], v3[1]], [v3[2], 0, -v3[0]], [-v3[1], v3[0], 0]])
...
def scalar_NFW_Hessian(x, y, z, logM, Rs, q, dirx, diry, dirz):
    ...
    hessian_matrix = hessian(potential_wrapper, argnums=(0, 1, 2))(
        x, y, z
    )  # Shape: (3, 3)
...
def vector_NFW_Hessian(x, y, z, logM, Rs, q, dirx, diry, dirz):
    ...
    hessian_fn = jax.vmap(
        hessian(potential_wrapper, argnums=(0, 1, 2)), in_axes=(0, 0, 0)
    )
...
def vector_Plummer_acceleration(x, y, z, logm, rs, x_origin=0, y_origin=0, z_origin=0):
    ...
    grad_fn = jax.vmap(
        grad(potential_wrapper, argnums=(0, 1, 2)), in_axes=(0, 0, 0, 0, 0, 0)
    )
...
def leapfrog_orbit_step(state, dt, logM, Rs, q, dirx, diry, dirz):
    ...
    ax_new, ay_new, az_new = scalar_NFW_acceleration(
        x_new, y_new, z_new, logM, Rs, q, dirx, diry, dirz
    )
...
def vectorized_leapfrog_orbit_step(states, dt, logM, Rs, q, dirx, diry, dirz):
    ...
    acc_new = vector_NFW_acceleration(
        x_new, y_new, z_new, logM, Rs, q, dirx, diry, dirz
    )
...
@jax.jit
def backward_integrate_orbit_leapfrog(
    x0, y0, z0, vx0, vy0, vz0, logM, Rs, q, dirx, diry, dirz, time
):
    ...
    _, trajectory = jax.lax.scan(
        step_fn, state, None, length=N_STEPS - 1, unroll=UNROLL
    )
    ...
    trajectory = jnp.vstack(
        [trajectory[::-1], jnp.array(state)[None, :]]
    )  # Shape: (MAX_LENGHT, 6)
...
def get_rj_vj_R(hessians, orbit_sat, mass_sat):
    ...
    rj = ((mass_sat * G / (Omega**2 - d2Phi_dr2)) * KPC_TO_KM**-2 + 1e-8) ** (
        1.0 / 3
    )  # kpc
...
def create_ic_particle_spray(orbit_sat, rj, vj, R, tail=0, key=random.PRNGKey(111)):
    ...
    rvy = (
        (
            random.normal(subkey_vy, shape=(N_PARTICLES // N_STEPS * N,)) * disp_vy
            + mean_vy
        )
        * vj
        * rx
    ...
    orbit_sat_repeated = jnp.repeat(
        orbit_sat, N_PARTICLES // N_STEPS, axis=0
    )  # More efficient than tile+reshape
    offset_pos_transformed = jnp.einsum("ni,nij->nj", offset_pos, R)
    offset_vel_transformed = jnp.einsum("ni,nij->nj", offset_vel, R)

    ic_stream = orbit_sat_repeated + jnp.concatenate(
...
def leapfrog_stream_step(state, dt, logM, Rs, q, dirx, diry, dirz, logm, rs):
    ...
    axp_new, ayp_new, azp_new = scalar_NFW_acceleration(
        xp_new, yp_new, zp_new, logM, Rs, q, dirx, diry, dirz
    )
    ...
    ax, ay, az = scalar_NFW_acceleration(
        x, y, z, logM, Rs, q, dirx, diry, dirz
    ) + scalar_Plummer_acceleration(
        x, y, z, logm, rs, x_origin=xp, y_origin=yp, z_origin=zp
    )  # km2 / s / Gyr / kpc
    ...
    ax_new, ay_new, az_new = scalar_NFW_acceleration(
        ...
        x_new, y_new, z_new, logm, rs, x_origin=xp_new, y_origin=yp_new, z_origin=zp_new
    )  # km2 / s / Gyr / kpc
...
@jax.jit
def vectorized_leapfrog_stream_step(
    states, dt, logM, Rs, q, dirx, diry, dirz, logm, rs
):
    ...
    acc_sat_new = vector_NFW_acceleration(
        xp_new, yp_new, zp_new, logM, Rs, q, dirx, diry, dirz
    )
    ...
    acc_nfw_new = vector_NFW_acceleration(
        x_new, y_new, z_new, logM, Rs, q, dirx, diry, dirz
    )
    acc_plummer_new = vector_Plummer_acceleration(
        x_new, y_new, z_new, logm, rs, xp_new, yp_new, zp_new
    )
...
@jax.jit
def forward_integrate_orbit_leapfrog(
    x0, y0, z0, vx0, vy0, vz0, logM, Rs, q, dirx, diry, dirz, time
):
    state = (x0, y0, z0, vx0, vy0, vz0)
    dt = time / N_STEPS
    ...
    # Correct concatenation
    trajectory = jnp.vstack(
        [jnp.array(state)[None, :], trajectory]
    )  # Shape: (MAX_LENGHT, 6)
...
@jax.jit
def forward_integrate_stream_leapfrog(
    index,
    x0,
    y0,
    z0,
    ...
    def step_fn(state, _):
        ...
        final_conditions = leapfrog_stream_step(
            initial_conditions, dt_here, logM, Rs, q, dirx, diry, dirz, logm, rs
        )
...
@jax.jit
def generate_stream(
    ic_particle_spray, xv_sat, logM, Rs, q, dirx, diry, dirz, logm, rs, time
):
    ...
    index = jnp.repeat(
        jnp.arange(0, N_STEPS, 1), N_PARTICLES // N_STEPS
    )  # Shape: (N_PARTICLES,)
    ...
    xv_stream = jax.vmap(
        ...
        in_axes=(
            ...
            0,
            0,
            0,  # map over each column of ic_particle_spray
            None,
            None,
            None,
            None,
...
@jax.jit
def generate_stream_vectorized(
    ic_particle_spray, xv_sat, logM, Rs, q, dirx, diry, dirz, logm, rs, time
):
    """
    ...
    def vectorized_step_fn(states, step_idx):
        ...
        new_states = vectorized_leapfrog_stream_step(
            states, dt_per_particle[0], logM, Rs, q, dirx, diry, dirz, logm, rs
        )
    ...
    final_states, trajectory = jax.lax.scan(
        vectorized_step_fn, initial_states, jnp.arange(N_STEPS - 1), unroll=UNROLL
    )
...
def bin_stream(theta_stream, r_stream, x_stream, y_stream, vz_stream, min_count):
    ...
    def per_bin_median(bin_idx, bin_ids, r, x, y, vz):
        ...
        return jax.lax.cond(
            count > min_count,
            compute_medians,
            lambda: (jnp.nan, jnp.nan, jnp.nan, jnp.nan, jnp.nan),
        )

    # Step 3: Vectorize
    all_bins = jnp.arange(1, N_BINS + 1)
    r_meds, w_meds, x_meds, y_meds, vz_meds = jax.vmap(
        per_bin_median, in_axes=(0, None, None, None, None, None)
...
def jax_stream_model(
    ...
    xv_sat, _ = backward_integrate_orbit_leapfrog(
        x0, y0, z0, vx0, vy0, vz0, logM, Rs, q, dirx, diry, dirz, time
    )
    theta_sat = jnp.arctan2(xv_sat[:, 1], xv_sat[:, 0])
    theta_sat = jnp.where(theta_sat < 0, theta_sat + 2 * jnp.pi, theta_sat)
    ...
    def true_branch(_):
        ...
        xv_sat_forward, _ = forward_integrate_orbit_leapfrog(
            xv_sat[0, 0],
            xv_sat[0, 1],
            xv_sat[0, 2],
            xv_sat[0, 3],
        ...
        algin_reference = theta_sat[-1] - theta_count[-1] * (
            2 * jnp.pi
        )  # Make sure the angle of reference is at theta=0

        final_theta_stream += (
            1 - jnp.sign(algin_reference - jnp.pi)
        ) / 2 * algin_reference + (1 + jnp.sign(algin_reference - jnp.pi)) / 2 * (
            algin_reference - 2 * jnp.pi
        )
        ...
        r_meds, w_meds, x_meds, y_meds, vz_meds = bin_stream(
            theta_stream, r_stream, x_stream, y_stream, vz_stream, min_count=min_count
        )
